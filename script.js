document.addEventListener('DOMContentLoaded', function () {
  ...
  if (contactForm) {
    const VALIDATORS = {
      ...
      phone: (value) => value.trim() === '' || /^[+\d\s\-().]{7,20}$/.test(value.trim())
    };

    function getErrorMessage(field, rule, param) {
      const label = field.closest('.form-group')?.querySelector('label')?.textContent?.trim() || field.name || 'This field';
      const messages = {
    ...
    function validateField(field) {
      ...
      for (const rule of rules) {
        ...
        if (validator && !validator(field.value, ruleParam)) {
    ...
    contactForm.addEventListener('blur', function (e) {
      ...
      if (field.matches('input, textarea, select') && field.type !== 'submit' && field.type !== 'button') {
        validateField(field);
      }
    }, true);
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '543dcf98-643a-4628-81e5-ab905c9ab980',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '543dcf98-643a-4628-81e5-ab905c9ab980';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '543dcf98-643a-4628-81e5-ab905c9ab980';
